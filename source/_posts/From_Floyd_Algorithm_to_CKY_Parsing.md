...
The context-free grammar $G$ (assumed to be in Chomsky Normal Form) contains two kinds of rules $A \rarr BC$ and $A \rarr t$. To generate $BC$ from $A$, there must exist a **split point $k$** such that $B$ yields the substring $w_{\{i...k\}}$ and $C$ yields the substring $w_{\{k...j\}}$. This logic is highly analogous to the intermediate node concept in the Floyd algorithm.

$T[i][j]=\bigcup_{k} \{ A \mid A \to BC,\ B \in T[i][k],\ C \in T[k][j] \}$
